...
public class SaveThemeDialogFragment extends DialogFragment {
    ...
    private boolean Show_save = true;
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        View v = View.inflate(ctx,
                org.cyanogenmod.theme.chooser2.R.layout.fragment_save_theme, null);
        mThemeName = (EditText) v.findViewById(org.cyanogenmod.theme.chooser2.R.id.theme_name);
        text_view = (TextView) v.findViewById(R.id.some_id_for_warn);
        text_view.setVisibility(View.GONE);
        mThemeName.setText(themeName);
        mThemeName.requestFocus();
        mThemeName.addTextChangedListener(mTextWatcher);
        mSaveProgress = (ProgressBar) v.findViewById
                (org.cyanogenmod.theme.chooser2.R.id.save_progress_indicator);
        alert.setView(v)
                .setPositiveButton
                        (org.cyanogenmod.theme.chooser2.R.string.dialog_save_theme_save_button_text,
                                null)
                .setNegativeButton
                      (org.cyanogenmod.theme.chooser2.R.string.dialog_save_theme_cancel_button_text,
    ...
    public void onStart() {
        ...
        mSaveButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                ContentResolver resolver_test = getActivity().getContentResolver();
                String name = mThemeName.getText().toString();
                String selection = "title=?";
                String[] selectionArgs ={name};
                Cursor cu = resolver_test.query(ThemesContract.ThemeMixColumns.CONTENT_URI,null,
                ...
                if(cu.getCount()>=1) {
                    ...
                    mThemeName.requestFocus();
                    mThemeName.setFocusable(true);
                    mThemeName.setFocusableInTouchMode(true);
                ...
                if(Show_save) {
                    new SaveThemeAsyncTask().execute(mThemeName.getText().toString(),
                            mCurrentTheme);
                    ...
                    if (fm.getFragments() != null) {
                        for (Fragment frag : fm.getFragments()) {
                            if (frag instanceof MyThemeFragment) {
                                ThemeFragment tf = ThemeFragment.newInstance
                                        (((MyThemeFragment) frag).getThemePackageName(), false);
                                ft.remove(frag);
                            }
                        }
                        ft.commit();
