...
public class ThemeMixFragment extends ThemeFragment {
    ...
    static ThemeMixFragment newInstance(String mixName, String baseThemePkgName, int id, boolean skipLoadingAnim) {
    ...
    protected void loadLockScreen(Cursor c, boolean animate) {
        ...
        if (bitmap != null) {
            ...
            String pkgName = isLiveLockScreen ? mSelectedComponentsMap.get(ThemesColumns.MODIFIES_LIVE_LOCK_SCREEN)
            ...
            if (!mPkgName.equals(pkgName) || (mPkgName.equals(pkgName)
                    ...
                    mBaseThemeSupportedComponents.contains(ThemesColumns.MODIFIES_LIVE_LOCK_SCREEN)))) {
                if (isLiveLockScreen) {
                    ...
                    setCardTitle(mLockScreenCard, pkgName, getString(org.cyanogenmod.theme.chooser.R.string.lockscreen_label));
                }
            }
        } else {
            // Set the lockscreen wallpaper to "None"
            mLockScreenCard.setWallpaper(null);
            setCardTitle(mLockScreenCard, WALLPAPER_NONE, getString(org.cyanogenmod.theme.chooser.R.string.lockscreen_label));
        }

        if (animate) {
            animateContentChange(org.cyanogenmod.theme.chooser.R.id.lockscreen_card, mLockScreenCard, overlay);
        }
    }
    ...
    protected void loadFont(Cursor c, boolean animate) {
        ...
        if (!mPkgName.equals(pkgName) || (mPkgName.equals(pkgName)
            ...
            setCardTitle(mFontCard, pkgName, getString(org.cyanogenmod.theme.chooser.R.string.font_label));
        }

        if (animate) {
            animateContentChange(org.cyanogenmod.theme.chooser.R.id.font_preview_container, mFontPreview, overlay);
        }
    }
    ...
    protected void loadAudible(int type, Cursor c, boolean animate) {
        if (mCurrentLoaderId == LOADER_ID_ALL) {
            ...
            if (playPause == null) {
                playPause = (ImageView) audibleContainer.findViewById(org.cyanogenmod.theme.chooser.R.id.play_pause);
            }
    ...
    @Override
    protected ThemeChangeRequest getThemeChangeRequestForComponents(Map<String, String> componentMap) {
        return super.getThemeChangeRequestForComponents(componentMap);
    }

    @Override
    protected ThemeChangeRequest getThemeChangeRequestForComponents(Map<String, String> componentMap, ThemeChangeRequest.RequestType requestType) {
    ...
    @Override
    public void setCurrentTheme(Map<String, String> currentTheme, MutableLong currentWallpaperComponentId) {
