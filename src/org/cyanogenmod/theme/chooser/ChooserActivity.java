...
public class ChooserActivity extends FragmentActivity
    ...
    private static int prev_count = 0;
    private boolean is_change_frag = false;
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        switch (loader.getId()) {
            ...
            case LOADER_ID_THEME_MIXES:
                if(data.getCount() > prev_count) {
                String name = null;
                    while(data.moveToLast()) {
                        final int titleIdx = data.getColumnIndex(ThemesContract.ThemeMixColumns.TITLE);
                        final String title = data.getString(titleIdx);
                        name = title;
                        break;
                    }
                mAppliedBaseTheme = name;
                PreferenceUtils.setAppliedBaseTheme(this,mAppliedBaseTheme);
                is_change_frag = true;
                }
