...
public class ChooserActivity extends FragmentActivity
    ...
    private boolean make_changes_to_fragments = false;
                ...
                if (mOrderedThemes != null) {
                    ...
                    if (mFragments.get(id) != null) {
                        ...
                        if (!themeItemInfo.isThemeMix) {
                            ...
                            if (themeItemInfo.is_theme_mix_applied_first) {
                                f = MyThemeFragment.newInstance(mAppliedBaseTheme, themeItemInfo.themeName,
                                        "Theme Mix", mAnimateContentIn,
                                        mShowLockScreenWallpaper);
                                wallpaperCmpntId = mCurrentWallpaperCmpntId;
                            } else {
...
//                                wallpaperCmpntId = mCurrentWallpaperCmpntId;
        ...
        public void updateInstalledThemes(Cursor c) {
            ...
            orderThemes();
            //make_changes_to_fragments = false;
            //notifyDataSetChanged();
        }

        public void updateThemeMixes(Cursor c) {
            mThemeMixes.clear();
            boolean is_new_theme_mix = false;
            //mAppliedThemeIndex = 1;

            if(c.getCount()>previous_count) {
                is_new_theme_mix = true;
                c.moveToPosition(-1);
                while(c.moveToLast()) {
                ...
                orderThemesDiff();
                make_changes_to_fragments = true;
                mFragments.clear();
                mAppliedBaseTheme = mAppliedBaseTheme+" ";
            }
            else if(c.getCount()==previous_count && is_new_theme_mix) {
                c.moveToPosition(-1);
                while(c.moveToLast()) {
                    final int titleIdx = c.getColumnIndex(ThemesContract.ThemeMixColumns.TITLE);
                    final int idIdx = c.getColumnIndex(ThemesContract.ThemeMixColumns._ID);
        ...
        private void orderThemesDiff() {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (themeItemInfo.packageName.equals(mAppliedBaseTheme) &&
                    ...
                    mAppliedThemeTitle = themeItemInfo.themeName;
                    mAppliedThemeAuthor = themeItemInfo.authorName;
                    mOrderedThemes.add(mThemeMixes.size()+1,themeItemInfo);
                }
                else {
                    mOrderedThemes.add(themeItemInfo);
                }
...
//                    mAppliedThemeAuthor = themeItemInfo.authorName;
//                    appliedThemeIndex = i;
//                    if(themeItemInfo.packageName.equals(ThemeConfig.SYSTEM_DEFAULT)) {
//                        mAppliedThemeIndex = 0;
//                    }
//                }
//                mOrderedThemes.add(themeItemInfo);
//            }
            ...
            // Now insert theme mixes after applied theme
//            N = mThemeMixes.size();
//            for (int i = 0; i < N; i++) {
//                ThemeItemInfo themeItemInfo = mThemeMixes.get(i);
//                if(themeItemInfo.packageName.equals(mAppliedBaseTheme)) {
...
//                    appliedThemeIndex = i;
//                }
//                mOrderedThemes.add(appliedThemeIndex + (i+1), themeItemInfo);
//            }
        }
        ...
        private class ThemeItemInfo {
            ...
            String authorName;
            boolean isThemeMix;
            boolean is_theme_mix_applied_first;
            int id;
            ...
            public ThemeItemInfo(String packageName, String themeName, String authorName,
                                 boolean isThemeMix, int id, boolean is_theme_mix_applied_first) {
                this.packageName = packageName;
                this.themeName = themeName;
                this.authorName = authorName;
                this.isThemeMix = isThemeMix;
                this.id = id;
                this.is_theme_mix_applied_first = is_theme_mix_applied_first;
            }
